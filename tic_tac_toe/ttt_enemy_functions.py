...
def change_difficulty(USERPATH):
    PATH_ICONS = USERPATH + "\\tic_tac_toe_icons\\"
    result = cmds.confirmDialog(title="tic tac toe",
    ...
    if result == "FAIR":
        difficulty = "FAIR"
        cmds.iconTextButton("ttt_difficulty", edit=True,
...
def check_difficulty_level(button, USERPATH):
    user_icon, ia_icon, idle_icon = get_paths_icons(USERPATH)
    difficulty = cmds.iconTextButton("ttt_difficulty", query=True,
                                     image=True)
    ...
    if "easy" in difficulty:
        cmds.iconTextButton(button,
                                    edit=True, image=user_icon)
        for button in range(0, 10):
            counter_button = "ttt_button0" + str(random.randint(1, 9))
            image_path = cmds.iconTextButton(counter_button,
